...
После magic-link входа гараж синхронизируется с Supabase, а гостевые диалоги
переносятся в аккаунт. VIN валидируется отдельно, шифруется сервером для
облачного хранения и не отправляется AI-модели.

Value-first лимит гостя за скользящие 24 часа: 3 новых диалога, 5 реальных
поисков и 30 ответов агента. История и уже найденные результаты остаются
доступными после лимита. Значения настраиваются через `GUEST_*_LIMIT_24H`.
