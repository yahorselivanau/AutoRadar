...
- один собственный Apify Actor;
- TypeScript;
- Crawlee;
- CheerioCrawler или прямой HTTP для простых страниц;
- PlaywrightCrawler только для источников, где без браузера нельзя получить выдачу.
...
## 4. Почему все адаптеры исполняются в Apify

Даже HTML-адаптеры запускаются внутри одного Actor. Это даёт:
...
Использование Apify не означает обязательный браузер. Большинство адаптеров должны быть обычными HTTP/HTML обработчиками.
...
- данные доступны публично, но генерируются только после взаимодействия.
...
## 17. Путь масштабирования

После подтверждения спроса:

1. официальные API и партнёрские фиды;
2. более длительное хранение предложений;
3. query cache;
4. очереди и Redis;
5. отдельные workers;
6. лицензированный каталог применяемости;
7. перенос Actor на VPS при экономической выгоде;
8. PostgreSQL можно оставить в Supabase или перенести стандартными средствами.
