...
диалога, активную машину и последний search job. Модель
`openai/gpt-5.4-nano` может использовать только зарегистрированные серверные
инструменты и ограничена шестью шагами. Web search не зарегистрирован.
...
сессиями на том же устройстве. После Supabase magic-link входа автомобили
...
- текущий model allowlist содержит только `openai/gpt-5.4-nano`;
...
- получает value-first лимит: 3 диалога, 5 search jobs и 30 AI-ответов за
  скользящие 24 часа;
