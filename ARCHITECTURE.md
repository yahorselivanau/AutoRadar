...
В текущем закрытом MVP web-поток вызывает только Zap.by. Гостевой гараж
...
предыдущий структурированный запрос, а модель возвращает его проверенную
обновлённую версию. Полный VIN извлекается и валидируется детерминированно и не
включается в prompt.

Облачная синхронизация гаража по Supabase Auth/RLS остаётся следующим этапом;
до неё интерфейс явно сообщает, что гостевые данные находятся только на
текущем устройстве. Для будущего серверного search-job потока прогресс получать
