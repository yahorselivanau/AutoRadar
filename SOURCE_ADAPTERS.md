...
| `zap.by`                      | новые детали              | разрешённый SSR-каталог по марке/модели; цены, наличие и сроки без логина                |            P0 | HTTP + Cheerio; `/search` и query/XHR не вызываются из-за robots.txt, поэтому OEM/VIN пока отключены.     |
...
Разрешённые path-based страницы `/carparts/...` возвращают SSR-карточки с
брендом, артикулом, BYN-ценой, наличием, сроком и исходной ссылкой. Маршруты
`/*search` и `/*?*` запрещены в `robots.txt`, поэтому адаптер поддерживает
только каталог по марке + модели + точному имени категории и не вызывает
обнаруженный picker XHR. Полное доказательство и ограничения:
