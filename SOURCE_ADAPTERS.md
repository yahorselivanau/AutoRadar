...
| `remzona.by`                  | новые детали              | публичный XHR-поиск по артикулу/названию возвращает реальные товарные карточки           |            P0 | Рабочий HTTP + Cheerio adapter; один запрос на поиск, без Playwright, proxy и стороннего scraping API.    |
